# T5 classical codec profiling — STM32F413ZH (NUCLEO-F413ZH)
...
2. In the **Board Selector**, choose **NUCLEO-F413ZH** (MCU **STM32F413ZHT6**).
...
On **NUCLEO-F413ZH**, the ST-Link USB virtual COM port is typically wired to **USART3**:
...
From this repository folder [`firmware/stm32f413zh_t5_profile/`](.), copy into your CubeIDE project:
...
Add the two `.c` files to the build. Include paths should already cover `Core/Inc`.
...
3. Ensure **USART3** is initialized before any `printf`.
...
- **Link errors for `_etext` / `_sdata` / `_edata`**: Your linker script may use different symbol names; adjust `t5_flash_estimate_bytes()` in `t5_profile_methods.c` to match the `.ld` file from Cube (or use the `.map` file).
